...
program.name('superscribe-extension').usage('[command] [options]');
program.version(getSdkVersion(), '-v, --version');

program
	.command('create')
	.arguments('<type> <name>')
	.description('Scaffold a new Superscribe extension')
	.option('-l, --language <language>', 'specify the language to use')
	.action(create);

program.command('add').description('Add entries to an existing Superscribe extension').action(add);

program
	.command('build')
	.description('Bundle a Superscribe extension to a single entrypoint')
...
program
	.command('link')
	.description('Creates a symlink to the extension in the Superscribe extensions folder')
	.argument('<path>', 'path to the extension folder of superscribe')
	.action(link);
