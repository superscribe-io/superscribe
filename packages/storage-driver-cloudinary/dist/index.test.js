...
beforeEach(() => {
    ...
    driver['getPublicId'] = vi.fn().mockImplementation((input) => {
        if (input === sample.path.srcFull)
            return sample.publicId.src;
        if (input === sample.path.destFull)
            return sample.publicId.dest;
        if (input === sample.path.inputFull)
            return sample.publicId.input;
        return '';
    });
...
describe('#toFormUrlEncoded', () => {
    ...
    beforeEach(() => {
        ...
        mockProps = Array.from(Array(3), () => randAlphaNumeric({ length: randNumber({ min: 2, max: 15 }) }).join(''));
        mockValues = randWord({ length: 3 });
    });
    ...
    test('Optionally sorts the properties alphabetically', () => {
        ...
        expect(driver['toFormUrlEncoded']({
            [mockProps[0]]: mockValues[0],
            [mockProps[1]]: mockValues[1],
            [mockProps[2]]: mockValues[2],
        }, { sort: true })).toBe(`${mockProps[2]}=${mockValues[2]}&${mockProps[0]}=${mockValues[0]}&${mockProps[1]}=${mockValues[1]}`);
    });
});
describe('#getFullSignature', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(createHash).mockReturnValue(mockCreateHash);
...
describe('#getPublicId', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(parse).mockReturnValue({ name: mockParsedPath });
    });
...
describe('#getBasicAuth', () => {
    ...
    beforeEach(() => {
        ...
        mockToString = vi.fn();
        vi.spyOn(Buffer, 'from').mockReturnValue({ toString: mockToString });
    });
...
describe('#read', () => {
    ...
    test('Calls fetch with generated URL', async () => {
        await driver.read(sample.path.input);
        expect(fetch).toHaveBeenCalledWith(`https://res.cloudinary.com/${sample.config.cloudName}/${sample.resourceType}/upload/${sample.parameterSignature}/${sample.path.inputFull}`, { method: 'GET' });
    });
    test('Adds optional Range header for start', async () => {
        await driver.read(sample.path.input, { start: sample.range.start });
        expect(fetch).toHaveBeenCalledWith(`https://res.cloudinary.com/${sample.config.cloudName}/${sample.resourceType}/upload/${sample.parameterSignature}/${sample.path.inputFull}`, { method: 'GET', headers: { Range: `bytes=${sample.range.start}-` } });
    });
    test('Adds optional Range header for end', async () => {
        await driver.read(sample.path.input, { end: sample.range.end });
        expect(fetch).toHaveBeenCalledWith(`https://res.cloudinary.com/${sample.config.cloudName}/${sample.resourceType}/upload/${sample.parameterSignature}/${sample.path.inputFull}`, { method: 'GET', headers: { Range: `bytes=-${sample.range.end}` } });
    });
    test('Adds optional Range header for start and end', async () => {
        await driver.read(sample.path.input, sample.range);
        expect(fetch).toHaveBeenCalledWith(`https://res.cloudinary.com/${sample.config.cloudName}/${sample.resourceType}/upload/${sample.parameterSignature}/${sample.path.inputFull}`, { method: 'GET', headers: { Range: `bytes=${sample.range.start}-${sample.range.end}` } });
    });
...
describe('#stat', () => {
    ...
    test('Fetches URL with url encoded body', async () => {
        await driver.stat(sample.path.input);
        expect(fetch).toHaveBeenCalledWith(`https://api.cloudinary.com/v1_1/${sample.config.cloudName}/${sample.resourceType}/explicit`, {
            method: 'POST',
            headers: {
...
describe('#move', () => {
    ...
    test('Fetches URL with url encoded body', async () => {
        await driver.move(sample.path.src, sample.path.dest);
        expect(fetch).toHaveBeenCalledWith(`https://api.cloudinary.com/v1_1/${sample.config.cloudName}/${sample.resourceType}/rename`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8',
            },
...
describe('#uploadChunk', () => {
    ...
    beforeEach(() => {
        ...
        input = {
            resourceType: sample.resourceType,
            blob: { size: randNumber({ min: 0, max: 1500 }) },
        ...
        vi.mocked(fetch).mockResolvedValue(mockResponse);
        vi.mocked(FormData).mockReturnValue(mockFormData);
    });
    ...
    test('Calls fetch with formData an range header', async () => {
        await driver['uploadChunk'](input);
        expect(fetch).toHaveBeenCalledWith(`https://api.cloudinary.com/v1_1/${sample.config.cloudName}/${sample.resourceType}/upload`, {
            method: 'POST',
            body: mockFormData,
            headers: {
...
describe('#delete', () => {
    ...
    test('Calls fetch with correct parameters', () => {
        ...
        expect(fetch).toHaveBeenCalledWith(`https://api.cloudinary.com/v1_1/${sample.config.cloudName}/${sample.resourceType}/destroy`, {
            method: 'POST',
            body: sample.formUrlEncoded,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8',
...
describe('#list', () => {
    ...
    test('Fetches search api results', async () => {
        await driver.list(sample.path.input).next();
        expect(fetch).toHaveBeenCalledWith(`https://api.cloudinary.com/v1_1/${sample.config.cloudName}/resources/search?expression=${sample.publicId.input}*&next_cursor=`, {
            method: 'GET',
            headers: {
    ...
    test('Keeps calling fetch as long as a next_cursor is returned', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(`https://api.cloudinary.com/v1_1/${sample.config.cloudName}/resources/search?expression=${sample.publicId.input}*&next_cursor=${mockNextCursor}`, {
            method: 'GET',
            headers: {
                Authorization: sample.basicAuth,
            },
