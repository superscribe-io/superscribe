---
description: A guide on how to build custom Modules in Superscribe.
readTime: 5 min read
---

# Custom Modules <small></small>

> Custom Modules are completely open-ended components that allow you to create new experiences within the Superscribe
...
imported by the Superscribe host.
...
`private-view` component to get access to Superscribe' page structure consisting of the module bar, the navigation, the
...
The Superscribe App uses Vue 3. There might be 3rd party libraries that aren't yet compatible with Vue 3.
...
by the `@superscribe/extensions-sdk` package. They can be used inside a `setup()` function like this:

```js
import { useApi, useStores } from '@superscribe/extensions-sdk';
...
The Superscribe App's Vue app instance provides a field called `api`, which can be injected into Vue components using
