...
Scheduling content has fewer steps for a dynamic site. Since you are calling your Superscribe API at the time that a
...
### Check the Published Date When Calling the Superscribe API
...
Using the [Superscribe JavaScript SDK](/reference/sdk) (preferred)

```js
const articles = await superscribe.items('articles').readByQuery({
	filter: {
		_and: [
			{
				status: {
...
```js
const response = await fetch('https://yoursuperscribeurl.com/items/articles?' + new URLSearchParams({
		filter: {
			_and: [
				{
					status: {
