---
description: Learn how to migrate your data model to a new Superscribe project using Hoppscotch.
tags: []
skill_level:
superscribe_version: 9.23.0
author_override:
author: Kevin Lewis
---
...
Superscribe' schema migration endpoints allow users to retrieve a project's data model and apply changes to another
project.

This is useful if you make changes to a data model in a development project and need to apply them to a production
project, or to move from a self-hosted project to Superscribe Cloud.

[Hoppscotch](https://hoppscotch.io/) is an open source API explorer. It is used in this recipe to make requests to your
Superscribe project's API without writing code.
...
You should have two Superscribe projects - this guide will refer to them as the "source" and the "target". Before starting,
make sure you have a static access token for both projects.

### Retrieve Data Model Snapshot From Source Project

![Screenshot of Hoppscotch annotated with numbers associated with the below points.](https://cdn.superscribe.io/docs/v9/cookbook/migration-hoppscotch/snapshot.webp)

1. Make sure `GET` is selected in the method dropdown.
2. In the URL field, enter your source Superscribe project URL followed by `/schema/snapshot`.
...
![Screenshot of Hoppscotch annotated with numbers associated with the below points.](https://cdn.superscribe.io/docs/v9/cookbook/migration-hoppscotch/diff.webp)
...
2. In the URL field, enter your target Superscribe project URL followed by `/schema/diff`.
...
![Screenshot of Hoppscotch annotated with numbers associated with the below points.](https://cdn.superscribe.io/docs/v9/cookbook/migration-hoppscotch/apply.webp)

1. Make sure `POST` is selected in the method dropdown.
2. In the URL field, enter your target Superscribe project URL followed by `/schema/apply`.
3. In the Parameters tab, set a query parameter called `access_token` with the access token for your source project.
...
The diff endpoint does not allow different Superscribe versions and database vendors by default. This is to avoid any
