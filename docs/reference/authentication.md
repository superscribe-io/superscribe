---
description: API documentation on authentication in Superscribe.
readTime: 5 min read
...
There are two types of tokens that can be used to authenticate within Superscribe.
...
server-to-server communication. They are saved as plain-text within `superscribe_users.token`.
