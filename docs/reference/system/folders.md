---
description: REST and GraphQL API documentation on the Folders collection in Superscribe.
readTime: 4 min read
...
List all folders that exist in Superscribe.
...
```graphql
type Query {
	folders_by_id(id: ID!): superscribe_folders
}
```
...
```graphql
type Mutation {
	create_folders_item(data: create_superscribe_folders_input): superscribe_folders
}
```
...
```graphql
type Mutation {
	create_folders_items(data: [create_superscribe_folders_input]): [superscribe_folders]
}
```
...
```graphql
type Mutation {
	update_folders_item(id: ID!, data: update_superscribe_folders_input): superscribe_folders
}
```
...
```graphql
type Mutation {
	update_folders_items(ids: [ID!]!, data: update_superscribe_folders_input): [superscribe_folders]
}
```
