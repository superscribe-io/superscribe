---
description: REST and GraphQL API documentation on the Fields collection in Superscribe.
readTime: 7 min read
...
`type` **string**\
The Superscribe data type of the field. See [Types](/getting-started/glossary#types) for possible options.

#### Meta

Superscribe metadata, primarily used in the Admin App. Meta is optional.

`id` **integer**\
Primary key of the metadata row in `superscribe_fields`.
...
are available for all drivers. Note: schema is optional. If a field exist in superscribe_fields, but not in the database,
...
`is_nullable` **boolean**\
Whether or not the column is nullable. This is what is used as the "required" state in Superscribe.
...
```graphql
type Query {
	fields_in_collection(collection: String!): superscribe_fields
}
```
...
```graphql
type Query {
	fields_by_name(collection: String!, field: String!): superscribe_fields
}
```
...
`type` **Required**\
One of the Superscribe types. This in turn controls what datatype is used for the column in the database. Setting the type to
...
```graphql
type Mutation {
	create_fields_item(collection: String!, data: create_superscribe_fields_input!): superscribe_fields
}
```
...
```graphql
type Mutation {
	update_fields_item(collection: String!, field: String!, data: update_superscribe_fields_input!): superscribe_fields
}
```
