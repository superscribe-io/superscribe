---
description: REST and GraphQL API documentation on the Webhooks collection in Superscribe.
readTime: 5 min read
...
List all webhooks that exist in Superscribe.
...
```graphql
type Query {
	webhooks_by_id(id: ID!): superscribe_webhooks
}
```
...
```graphql
type Mutation {
	create_webhooks_item(data: create_superscribe_webhooks_input!): superscribe_webhooks
}
```
...
```graphql
type Mutation {
	create_webhooks_items(data: [create_superscribe_webhooks_input!]!): [superscribe_webhooks]
}
```
...
```graphql
type Mutation {
	update_webhooks_item(id: ID!, data: update_superscribe_webhooks_input!): superscribe_webhooks
}
```
...
```graphql
type Mutation {
	update_webhooks_items(ids: [ID!]!, data: update_superscribe_webhooks_input!): [superscribe_webhooks]
}
```
