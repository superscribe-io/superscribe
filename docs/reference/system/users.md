---
description: REST and GraphQL API documentation on the Users collection in Superscribe.
readTime: 9 min read
...
> Superscribe Users are the individual accounts that let you authenticate into the API and App. Each user belongs to a Role
...
`language` **string**\
Language the Admin App is rendered in. See [our Crowdin page](https://locales.superscribe.io) for all available languages and
translations.
...
List all users that exist in Superscribe.
...
```graphql
type Query {
	users_by_id(id: ID!): superscribe_users
}
```
...
```graphql
type Mutation {
	update_users_me(data: update_superscribe_users_input!): superscribe_users
}
```
...
```graphql
type Mutation {
	create_users_item(data: create_superscribe_users_input!): superscribe_users
}
```
...
```graphql
type Mutation {
	create_users_items(data: [create_superscribe_users_input!]!): [superscribe_users]
}
```
...
```graphql
type Mutation {
	update_users_item(id: ID!, data: update_superscribe_users_input!): superscribe_users
}
```
...
```graphql
type Mutation {
	update_users_items(ids: [ID!]!, data: update_superscribe_users_input!): [superscribe_users]
}
```
