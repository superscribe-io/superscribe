---
description: REST and GraphQL API documentation on the "Collections" collection in Superscribe.
readTime: 5 min read
...
Superscribe metadata, primarily used in the Admin App.
...
```graphql
type Query {
	collections: [superscribe_collections]
}
```
...
```graphql
type Query {
	collections_by_name(name: String!): superscribe_collections
}
```
...
```graphql
type Mutation {
	create_collections_item(data: superscribe_collections): superscribe_collections
}
```
...
```graphql
type Mutation {
	update_collections_item(collection: String!, data: update_superscribe_collections_input!): superscribe_collections
}
```
