---
description: REST and GraphQL API documentation on the Notifications collection in Superscribe.
readTime: 5 min read
...
List all notifications that exist in Superscribe.
...
```graphql
type Query {
	notifications: [superscribe_notifications]
}
```
...
```graphql
type Query {
	notifications_by_id(id: ID!): superscribe_notifications
}
```
...
```graphql
type Mutation {
	create_notifications_item(data: create_superscribe_notifications_input!): superscribe_notifications
}
```
...
```graphql
type Mutation {
	create_notifications_items(data: [create_superscribe_notifications_input!]!): [superscribe_notifications]
}
```
...
```graphql
type Mutation {
	update_notifications_item(id: ID!, data: update_superscribe_notifications_input): superscribe_notifications
}
```
...
```graphql
type Mutation {
	update_notifications_items(ids: [ID!]!, data: update_superscribe_notifications_input): [superscribe_notifications]
}
```
