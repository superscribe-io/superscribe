---
description: REST and GraphQL API documentation on the Permissions collection in Superscribe.
readTime: 5 min read
...
List all permissions that exist in Superscribe.
...
```graphql
type Query {
	permissions: superscribe_permissions
}
```
...
```graphql
type Query {
	permissions_by_id(id: ID!): superscribe_permissions
}
```
...
```graphql
type Mutation {
	create_permissions_item(data: create_superscribe_permissions_input!): superscribe_permissions
}
```
...
```graphql
type Mutation {
	create_permissions_items(data: [create_superscribe_permissions_input!]!): [superscribe_permissions]
}
```
...
```graphql
type Mutation {
	update_permissions_item(id: ID!, data: update_superscribe_permissions_input!): superscribe_permissions
}
```
...
```graphql
type Mutation {
	update_permissions_items(id: [ID!]!, data: update_superscribe_permissions_input!): [superscribe_permissions]
}
```
