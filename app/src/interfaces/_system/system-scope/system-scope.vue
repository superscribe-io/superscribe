...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const options = computed(() => {
			let options: any[] = [
				...
				},
				{
					...
					value: 'superscribe_users',
				},
				{
					text: t('role') + ': ' + t('select'),
					value: 'superscribe_roles',
				},
			];
		...
		function onSelectItem(value: string[]) {
			if (collection.value === 'superscribe_users') return emit('input', 'user_' + value[0]);
			if (collection.value === 'superscribe_roles') return emit('input', 'role_' + value[0]);
		}
