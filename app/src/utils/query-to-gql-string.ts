...
import { Filter, Query } from '@superscribe/types';
...
export function formatQuery({ collection, query }: QueryInfo): Record<string, any> {
	...
	const alias = collection.startsWith('superscribe_') ? collection.substring(9) : collection;
...
/**
 * Replace functions from Superscribe-Filter format to GraphQL format
 */
