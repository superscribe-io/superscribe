<template>
	...
	>
		...
		>
			...
			<template #actions>
				<search-input v-model="search" v-model:filter="filter" collection="superscribe_users" />
...
import { useLayout } from '@superscribe/composables';
import { Role } from '@superscribe/types';
import { mergeFilters } from '@superscribe/utils';
...
const { layout, layoutOptions, layoutQuery, filter, search, resetPreset } = usePreset(ref('superscribe_users'));
...
const canInviteUsers = computed(() => {
	...
	const usersCreatePermission = permissionsStore.permissions.find(
		(permission) => permission.collection === 'superscribe_users' && permission.action === 'create'
	);

	const rolesReadPermission = permissionsStore.permissions.find(
		(permission) => permission.collection === 'superscribe_roles' && permission.action === 'read'
	);
...
function usePermissions() {
	const batchEditAllowed = computed(() => {
		...
		const updatePermissions = permissionsStore.permissions.find(
			(permission) => permission.action === 'update' && permission.collection === 'superscribe_users'
		);
	...
	const batchDeleteAllowed = computed(() => {
		...
		const deletePermissions = permissionsStore.permissions.find(
			(permission) => permission.action === 'delete' && permission.collection === 'superscribe_users'
		);
	...
	const createAllowed = computed(() => {
		...
		const createPermissions = permissionsStore.permissions.find(
			(permission) => permission.action === 'create' && permission.collection === 'superscribe_users'
		);
