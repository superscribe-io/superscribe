...
import { Preset, Filter } from '@superscribe/types';
import api from '@/api';
...
function useForm() {
	const systemCollectionWhiteList = ['superscribe_users', 'superscribe_files', 'superscribe_activity'];
			...
			meta: {
				...
				options: {
					choices: collectionsStore.collections
						...
						.filter((option) => {
							if (option.value.startsWith('superscribe_')) return systemCollectionWhiteList.includes(option.value);
