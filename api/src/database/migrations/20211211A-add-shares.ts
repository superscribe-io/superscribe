...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable('superscribe_shares', (table) => {
		table.uuid('id').primary().notNullable();
		table.string('name');
		table.string('collection', 64).references('collection').inTable('superscribe_collections').onDelete('CASCADE');
		table.string('item');
		table.uuid('role').references('id').inTable('superscribe_roles').onDelete('CASCADE');
	...
	await knex.schema.alterTable('superscribe_sessions', (table) => {
		...
		table.uuid('share').references('id').inTable('superscribe_shares').onDelete('CASCADE');
	});
}
...
export async function down(knex: Knex): Promise<void> {
	...
	await knex.schema.dropTable('superscribe_shares');
}
