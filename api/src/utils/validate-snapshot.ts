...
export function validateSnapshot(snapshot: Snapshot, force = false) {
	...
	if (snapshot.superscribe !== currentSuperscribeVersion) {
		throw new InvalidPayloadException(
			`Provided snapshot's superscribe version ${snapshot.superscribe} does not match the current instance's version ${currentSuperscribeVersion}. You can bypass this check by passing the "force" query parameter.`
		);
	}
