...
import type { Accountability, Aggregate, Filter, PrimaryKey, Query, SchemaOverview } from '@superscribe/types';
...
export class GraphQLService {
	...
	getSchema(type: 'schema' | 'sdl' = 'schema'): GraphQLSchema | string {
		...
		const scopeFilter = (collection: SchemaOverview['collections'][string]) => {
			if (this.scope === 'items' && collection.collection.startsWith('superscribe_') === true) return false;

			if (this.scope === 'system') {
				if (collection.collection.startsWith('superscribe_') === false) return false;
	...
	async resolveQuery(info: GraphQLResolveInfo): Promise<Partial<Item> | null> {
		...
		if (this.scope === 'system') collection = `superscribe_${collection}`;
			...
			schemaComposer.Mutation.addFields({
				import_file: {
					...
					args: {
						...
						data: toInputObjectType(CreateCollectionTypes['superscribe_files']!).setTypeName(
							'create_superscribe_files_input'
						),
					},
					resolve: async (_, args, __, info) => {
