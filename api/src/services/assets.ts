...
export class AssetsService {
	...
	async getAsset(
		 ...
		 * with a wrong type. In case of superscribe_files where id is a uuid, we'll have to verify the
		...
		if (systemPublicKeys.includes(id) === false && this.accountability?.admin !== true) {
			await this.authorizationService.checkAccess('read', 'superscribe_files', id);
		}

		const file = (await this.knex.select('*').from('superscribe_files').where({ id }).first()) as File;
