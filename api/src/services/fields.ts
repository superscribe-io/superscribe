...
import type { Accountability, Field, FieldMeta, RawField, SchemaOverview, Type } from '@superscribe/types';
...
export class FieldsService {
	...
	async readAll(collection?: string): Promise<Field[]> {
		...
		const aliasQuery = this.knex.select<any[]>('*').from('superscribe_fields');
	...
	async updateField(collection: string, field: RawField, opts?: MutationOptions): Promise<string> {
		...
		try {
			...
			if (hookAdjustedField.schema) {
				...
				if (!isEqual(sanitizeColumn(existingColumn), sanitizeColumn(hookAdjustedField.schema as Column))) {
					try {
