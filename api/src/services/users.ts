...
export class UsersService extends ItemsService {
	...
	private inviteUrl(email: string, url: string | null): string {
		...
		const token = jwt.sign(payload, env['SECRET'] as string, { expiresIn: '7d', issuer: 'superscribe' });
	...
	async requestPasswordReset(email: string, url: string | null, subject?: string | null): Promise<void> {
		...
		const token = jwt.sign(payload, env['SECRET'] as string, { expiresIn: '1d', issuer: 'superscribe' });
	...
	async resetPassword(token: string, password: string): Promise<void> {
		const { email, scope, hash } = jwt.verify(token, env['SECRET'] as string, { issuer: 'superscribe' }) as {
			email: string;
			scope: string;
			hash: string;
		};
