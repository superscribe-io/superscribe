...
test.each([
    ...
    await config.handler({ permissions }, { accountability: testAccountability, getSchema });
...
test.each([
    ...
    await config.handler({ recipient: testRecipient, message }, { accountability: testAccountability, getSchema });
    expect(vi.mocked(NotificationsService).prototype.createMany).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ message: expected })]));
});
