...
test.each([
    ...
    await config.handler({ collection: testCollection, permissions }, { accountability: testAccountability, getSchema });
...
test.each([
    ...
    await config.handler({ collection: testCollection, payload }, { accountability: testAccountability, getSchema });
    if (expected) {
...
test('should emit events when true', async () => {
    await config.handler({ collection: testCollection, payload: {}, emitEvents: true }, { accountability: testAccountability, getSchema });
...
test.each([undefined, false])('should not emit events when %s', async (emitEvents) => {
    await config.handler({ collection: testCollection, payload: {}, emitEvents }, { accountability: testAccountability, getSchema });
