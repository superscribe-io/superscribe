...
describe('Operations / Item Delete', () => {
    ...
    test.each([
        ...
        await config.handler({ collection: testCollection, query: testQuery, permissions }, { accountability: testAccountability, getSchema });
        expect(ItemsService).toHaveBeenCalledWith(testCollection, expect.objectContaining({ schema: {}, accountability: expected, knex: undefined }));
    });
    test('should have fallback when query is not defined', async () => {
        await config.handler({ collection: testCollection }, { accountability: testAccountability, getSchema });
    ...
    test.each([undefined, []])('should call deleteByQuery with correct query when key is $payload', async (key) => {
        await config.handler({ collection: testCollection, query: testQuery, key }, { accountability: testAccountability, getSchema });
    ...
    test('should emit events for deleteByQuery when true', async () => {
        await config.handler({ collection: testCollection, query: testQuery, emitEvents: true }, { accountability: testAccountability, getSchema });
    ...
    test.each([undefined, false])('should not emit events for deleteByQuery when %s', async (emitEvents) => {
        await config.handler({ collection: testCollection, query: testQuery, emitEvents }, { accountability: testAccountability, getSchema });
    ...
    test.each([1, [1]])('should call deleteOne when key is $payload', async (key) => {
        await config.handler({ collection: testCollection, key }, { accountability: testAccountability, getSchema });
    ...
    test('should emit events for deleteOne when true', async () => {
        const key = 1;
        await config.handler({ collection: testCollection, key, emitEvents: true }, { accountability: testAccountability, getSchema });
    ...
    test.each([undefined, false])('should not emit events for deleteOne when %s', async (emitEvents) => {
        const key = 1;
        await config.handler({ collection: testCollection, key, emitEvents }, { accountability: testAccountability, getSchema });
    ...
    test('should call deleteMany when key is an array with more than one item', async () => {
        await config.handler({ collection: testCollection, key: [1, 2, 3] }, { accountability: testAccountability, getSchema });
        expect(ItemsService.prototype.deleteByQuery).not.toHaveBeenCalled();
        expect(ItemsService.prototype.deleteOne).not.toHaveBeenCalled();
    ...
    test('should emit events for deleteMany when true', async () => {
        const keys = [1, 2, 3];
        await config.handler({ collection: testCollection, key: keys, emitEvents: true }, { accountability: testAccountability, getSchema });
    ...
    test.each([undefined, false])('should not emit events for deleteMany when %s', async (emitEvents) => {
        const keys = [1, 2, 3];
        await config.handler({ collection: testCollection, key: keys, emitEvents }, { accountability: testAccountability, getSchema });
