...
vi.mock('../../utils/get-accountability-for-role.js', () => ({
    getAccountabilityForRole: vi.fn((role, _context) => Promise.resolve(role)),
}));
...
describe('Operations / Item Update', () => {
    ...
    test.each([
        ...
        await config.handler({ collection: testCollection, payload: testPayload, permissions }, { accountability: testAccountability, getSchema });
        expect(vi.mocked(ItemsService)).toHaveBeenCalledWith(testCollection, expect.objectContaining({ schema: {}, accountability: expected, knex: undefined }));
    });
    test('should return null when payload is not defined', async () => {
        const result = await config.handler({ collection: testCollection }, { accountability: testAccountability, getSchema });
    ...
    test.each([undefined, []])('should call updateByQuery with correct query when key is $payload', async (key) => {
        const query = { limit: -1 };
        await config.handler({ collection: testCollection, payload: testPayload, query, key }, { accountability: testAccountability, getSchema });
    ...
    test('should emit events for updateByQuery when true', async () => {
        const query = { limit: -1 };
        await config.handler({ collection: testCollection, payload: testPayload, query, emitEvents: true }, { accountability: testAccountability, getSchema });
    ...
    test.each([undefined, false])('should not emit events for updateByQuery when %s', async (emitEvents) => {
        const query = { limit: -1 };
        await config.handler({ collection: testCollection, payload: testPayload, query, emitEvents }, { accountability: testAccountability, getSchema });
        expect(vi.mocked(ItemsService).prototype.updateByQuery).toHaveBeenCalledWith(query, testPayload, {
            emitEvents: false,
        });
    });
    test.each([1, [1]])('should call updateOne when key is $payload', async (key) => {
        await config.handler({ collection: testCollection, payload: testPayload, key }, { accountability: testAccountability, getSchema });
        expect(vi.mocked(ItemsService).prototype.updateByQuery).not.toHaveBeenCalled();
    ...
    test('should emit events for updateOne when true', async () => {
        const key = 1;
        await config.handler({ collection: testCollection, payload: testPayload, key, emitEvents: true }, { accountability: testAccountability, getSchema });
    ...
    test.each([undefined, false])('should not emit events for updateOne when %s', async (emitEvents) => {
        const key = 1;
        await config.handler({ collection: testCollection, payload: testPayload, key: key, emitEvents }, { accountability: testAccountability, getSchema });
    ...
    test('should call updateMany when key is an array with more than one item', async () => {
        await config.handler({ collection: testCollection, payload: testPayload, key: [1, 2, 3] }, { accountability: testAccountability, getSchema });
        expect(vi.mocked(ItemsService).prototype.updateByQuery).not.toHaveBeenCalled();
        expect(vi.mocked(ItemsService).prototype.updateOne).not.toHaveBeenCalled();
    ...
    test('should emit events for updateMany when true', async () => {
        const keys = [1, 2, 3];
        await config.handler({ collection: testCollection, payload: testPayload, key: keys, emitEvents: true }, { accountability: testAccountability, getSchema });
    ...
    test.each([undefined, false])('should not emit events for updateMany when %s', async (emitEvents) => {
        const keys = [1, 2, 3];
        await config.handler({ collection: testCollection, payload: testPayload, key: keys, emitEvents }, { accountability: testAccountability, getSchema });
