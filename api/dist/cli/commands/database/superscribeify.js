...
export default async function superscribeify() {
    ...
    try {
        await knex.transaction(async (trx) => {
            ...
            let table_names = result.rows.map((row) => row.table_name);
            ...
            for (let table_name of renamed_tables) {
                ...
                `);
                let column_names = result.rows.map((row) => row.column_name);
                let rename_columns = column_names.filter((name) => name.includes('directus'));
