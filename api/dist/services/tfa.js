...
export class TFAService {
    ...
    constructor(options) {
        ...
        this.itemsService = new ItemsService('superscribe_users', options);
    }
    async verifyOTP(key, otp, secret) {
        if (secret) {
            return authenticator.check(otp, secret);
        }
        const user = await this.knex.select('tfa_secret').from('superscribe_users').where({ id: key }).first();
        if (!user?.tfa_secret) {
    ...
    async generateTFA(key) {
        const user = await this.knex.select('email', 'tfa_secret').from('superscribe_users').where({ id: key }).first();
        ...
        const project = await this.knex.select('project_name').from('superscribe_settings').limit(1).first();
        return {
            secret,
            url: authenticator.keyuri(user.email, project?.project_name || 'Superscribe', secret),
        };
    }
