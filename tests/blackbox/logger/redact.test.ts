...
describe('Logger Redact Tests', () => {
	...
	const superscribeInstances = {} as { [vendor: string]: ChildProcess };
	...
	beforeAll(async () => {
		...
		for (const vendor of vendors) {
			...
			superscribeInstances[vendor] = server;

			promises.push(awaitSuperscribeConnection(Number(env[vendor].PORT)));
		}
	...
	afterAll(async () => {
		for (const [vendor, connection] of databases) {
			superscribeInstances[vendor]!.kill();
				...
				common.TEST_USERS.forEach((userKey) => {
					describe(common.USER[userKey].NAME, () => {
						it.each(vendors)('%s', async (vendor) => {
							...
							// Action
							const logger = new TestLogger(superscribeInstances[vendor], '/auth/refresh', true);
							...
							const loggerGql = new TestLogger(superscribeInstances[vendor], '/graphql/system', true);
			...
			describe.each(authModes)('for %s mode', (mode) => {
				common.TEST_USERS.forEach((userKey) => {
					describe(common.USER[userKey].NAME, () => {
						it.each(vendors)('%s', async (vendor) => {
							// Setup
							const cookieName = 'superscribe_refresh_token';
