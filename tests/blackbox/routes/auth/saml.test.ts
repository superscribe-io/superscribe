...
describe('/auth/login/saml', () => {
	...
	describe('POST /acs', () => {
		describe('when no redirect is provided', () => {
			describe('returns superscribe refresh token in JSON', () => {
				it.each(vendors)('%s', async (vendor) => {
					// Action
		...
		describe('when redirect is provided', () => {
			describe('returns superscribe refresh token in cookie', () => {
				it.each(vendors)('%s', async (vendor) => {
					// Action
					...
					// Assert
					expect(cookies).toMatch(/superscribe_refresh_token/);
				});
			});
		});
	});
