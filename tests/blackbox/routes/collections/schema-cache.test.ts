...
import { awaitSuperscribeConnection } from '@utils/await-connection';
...
describe('Schema Caching Tests', () => {
	...
	const tzSuperscribe = {} as { [vendor: string]: ChildProcess[] };
	...
	beforeAll(async () => {
		...
		for (const vendor of vendors) {
			...
			env1[vendor].CACHE_NAMESPACE = 'superscribe-schema-cache';
			env1[vendor].MESSENGER_STORE = 'redis';
			env1[vendor].MESSENGER_NAMESPACE = `superscribe-${vendor}`;
			...
			tzSuperscribe[vendor] = [server1, server2, server3, server4];
			envs[vendor] = [env1, env2, env3, env4];

			promises.push(awaitSuperscribeConnection(newServerPort1));
			promises.push(awaitSuperscribeConnection(newServerPort2));
			promises.push(awaitSuperscribeConnection(newServerPort3));
			promises.push(awaitSuperscribeConnection(newServerPort4));
		}
	...
	afterAll(async () => {
		for (const [vendor, connection] of databases) {
			for (const instance of tzSuperscribe[vendor]!) {
				instance.kill();
			}
