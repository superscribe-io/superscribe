...
declare global {
	var superscribeFlowDataServerUrl: string;
...
class CustomEnvironment extends NodeEnvironment {
	...
	async setup() {
		...
		this.global.superscribeFlowDataServerUrl = serverUrl;
		...
		while (testIndex !== 0) {
			try {
				const response = await axios.get(`${this.global.superscribeFlowDataServerUrl}/items/tests_flow_completed`, {
					params: {
						'aggregate[count]': 'id',
					},
					headers: {
	...
	async teardown() {
		...
		await axios.post(`${this.global.superscribeFlowDataServerUrl}/items/tests_flow_completed`, body, {
			headers: {
				Authorization: `Bearer ${common.USER.TESTS_FLOW.TOKEN}`,
