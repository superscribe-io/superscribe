...
import { awaitDatabaseConnection, awaitSuperscribeConnection } from '../utils/await-connection';
...
export default async (): Promise<void> => {
	...
	await new Listr([
						...
						return {
							...
							task: async () => {
								...
								if (bootstrap.stderr.length > 0) {
									throw new Error(`Superscribe-${vendor} bootstrap failed: \n ${bootstrap.stderr.toString()}`);
								}
								...
								if (!process.env.TEST_LOCAL) {
									...
									global.superscribe[vendor] = server;
									let serverOutput = '';
									...
									server.on('exit', (code) => {
										...
										if (code !== null) throw new Error(`Superscribe-${vendor} server failed: \n ${serverOutput}`);
									});

									// Give the server some time to start
									await awaitSuperscribeConnection(Number(config.envs[vendor]!.PORT!));
									server.on('exit', () => undefined);

									// Set up separate superscribe instance without system cache
									...
									global.superscribeNoCache[vendor] = serverNoCache;
									...
									serverNoCache.on('exit', (code) => {
										...
										if (code !== null)
											throw new Error(`Superscribe-${vendor}-no-cache server failed: \n ${serverNoCacheOutput}`);
									});

									// Give the server some time to start
									await awaitSuperscribeConnection(Number(noCacheEnv.PORT!));
				...
				return new Listr([
					{
						...
						task: async () => {
							...
							if (!process.env.serverUrl) {
								throw new Error('Unable to connect to any superscribe server');
							}
						},
					},
				]);
		...
		.run()
		.catch((reason) => {
			for (const server of Object.values(global.superscribe)) {
				server?.kill();
			}

			for (const serverNoCache of Object.values(global.superscribeNoCache)) {
				serverNoCache?.kill();
			}
